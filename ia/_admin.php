...
/* pour supprimer les pdf de la page docs */
if(isset($_GET["lid"]) && ctype_digit($_GET["lid"])){
    ...
    mysqli_query($connect,"DELETE FROM fichier WHERE id =".$_GET["lid"])or die("Zut!");
        ...
        <article>
            ...
            <div>
                ...
                while ($lesdocs = mysqli_fetch_assoc($recup_docs)) {
                    ...
                    echo ")<br />";
                    echo "<iframe  width='50%' height='200px' src='" . $lesdocs['lurl'] . "'><p><a href='" . $lesdocs['lurl'] . "' title='Calendrier des dimanches'><img src='" . $lesdocs['lurl'] . "' alt='Calendrier des dimanches'/></a></p></iframe><img src='../img/supprimer.png' 
                      onclick='supprime(\""
                        .substr(strrchr($image['lurl'], '/'),1)."\""
                        . ", "
                        .$image['id'].");' alt='Supprimer' /><br />";
                }
                ?>
        </article>   
    </section>
