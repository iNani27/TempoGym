...
if(isset($_FILES['lefichier'])&& isset($_POST['letitre'])){
    ...
    if(move_uploaded_file(
            $_FILES['lefichier']['tmp_name'],
            $dossier.$nom.$extension)){
       
    mysqli_query($connect,"INSERT INTO tgj_docs 
                            (lurl,letitre,lepoids,tgi_pages_id)
                            VALUES
                            ('$dossier$nom$extension','$letitre',$taille_fichier,8);
...
$recup_dim = mysqli_query($connect, "SELECT * FROM tgj_docs WHERE tgi_pages_id=8;") or die(mysqli_error($connect));